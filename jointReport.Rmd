...
df.tidy <- df %>% 
  ...
  gather(owner, response, ends_with("_baseline"), 
         ends_with("_post"), ends_with("_change")) %>%
  separate(owner, into = c("owner", "scoringPeriod"))
```
...
### Sanity checks on subject inclusion

There should be 60 participants, and there are. 

```{r}
length(unique(df.tidy$participant_id))
```
...
```{r}
mean(df.tidy$response[df.tidy$prime_type == "self" & 
                          df.tidy$owner == "child" & 
                          df.tidy$scoringPeriod == "change"])

mean(df.tidy$response[df.tidy$prime_type == "self" & 
                          df.tidy$owner == "experimenter" & 
                          df.tidy$scoringPeriod == "change"])
```
...
```{r}
t.test(df.tidy$response[df.tidy$prime_type == "self" & 
                          df.tidy$owner == "child" & 
                          df.tidy$scoringPeriod == "change"], 
       df.tidy$response[df.tidy$prime_type == "self" & 
                          df.tidy$owner == "experimenter" & 
                          df.tidy$scoringPeriod == "change"], paired = TRUE)
```
...
There was no significant effect of toy owner in the neutral-focus condition (Mchild = 0.89, 95% CIchild = [0.31, 1.46], Mexperimenter = 0.71, 95% CIexperimenter = [0.04, 1.38], t(19) = 0.48, p = 0.64, d = 0.13). The mean value change scores for each condition as a function of toy owner are presented in Fig. 2. We found no significant main effects of types of picture type (Mself = 0.44, 95% CIself = [−0.04, 0.91], Mother = 0.51, 95% CIother = [0.04, 0.98], Mneutral = 0.80, 95% CIneutral = [0.32, 1.28], F(2, 53) = 0.61, p = 0.55, View the MathML source) or toy owner (Mchild = 0.67, 95% CIchild = [0.34, 0.99], Mexperimenter = 0.50, 95% CIexperimenter = [0.12, 0.89], F(1, 53) = 0.55, p = 0.46, View the MathML source). A significant effect of age was observed (F(1, 53) = 7.68, p = 0.01, View the MathML source), indicating that older children increased their valuations more than younger children regardless of toy owner or picture condition. No significant effect of gender (F(1, 53) = 0.46, p = 0.50, View the MathML source), and no significant interactions involving gender or age were observed (Fs < 2.69, ps > 0.11, etc.).
...
df %>% 
  group_by(prime_type) %>% 
  summarise(mean=mean(to.n(child_change), na.rm=TRUE))
